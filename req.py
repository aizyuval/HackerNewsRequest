...
try:
    while index > item_limit:
        ...
        try:
            with FuturesSession(
                session=ss
            ) as session:   # Utilizing FuturesSessions to hold a bunch of requests to do at once

                try:
                    futures = [
                        session.get(
                            f'https://hacker-news.firebaseio.com/v0/item/{_}.json'
                        )
                        for _ in range(index - item_batch_size + 1, index + 1)
                    ]
                except Exception as e:
                    logging.error(
                        f'SGETFAIL on item between: {index-item_batch_size} and: {index} SKIPBATCH Exception: {e}'
                    )
                ...
                for future in as_completed(futures):
                    ...
                    if (
                        'url' in response and 'title' in response
                    ):   # verify it is a story/show_hn
                        ...
                        try:
                            ...
                            logging.info(
                                f'INSSUCCESS statement item: ({index}) execution successful'
                            )
                        except psycopg2.Error as e:
                            logging.error(
                                f'INSFAIL statement item: ({index}) excecution FAILED with exception: {e}'
                            )
            ...
            logging.error(
                f'PARSEFAIL on item between: {index-item_batch_size} and: {index} SKIPBATCH ROLLBACK Exception: {e}'
            )
        ...
        try:
            ...
            logging.info(
                f'Commit number: {commit} successful. item processed: {maxitem-index} Inserted items count: {insert_global_count} '
            )
            ...
            logging.info(
                f'CMTFAIL Commit number: {commit} FAILED with exception: {e}'
            )

except KeyboardInterrupt:

    logging.info(
        f"STOPSCRIPT got interrupt. Didn't commited after item: {p_item}"
    )
    ...
    logging.error(
        f"SCRIPTERR got interrupt. Didn't commited after item: {p_item}. Exception: {e}"
    )
    conn.rollback()
    cur.close()
    conn.close()
